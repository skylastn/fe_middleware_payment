    ...
    return (
      <CheckoutLayout title="Pilih Pembayaran">
        <div className="flex flex-col items-center justify-center py-24 space-y-4">
          ...
          <p className="text-xs font-medium text-slate-500">Memuat saluran pembayaran...</p>
        </div>
      </CheckoutLayout>
    );
  }
      ...
      <div className="space-y-5">
        {categories.length === 0 ? (
          <div className="p-8 text-center border border-dashed border-slate-200 rounded-xl space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-700">Tidak ada saluran pembayaran aktif</p>
            <p className="text-xs text-slate-400">Silakan hubungi pemilik merchant untuk mengaktifkan saluran.</p>
          </div>
        ) : (
          categories.map((category) => (
            <div key={category.id || category.key} className="space-y-2.5">
              <div className="px-1">
                ...
                {category.detail && (
                  <p className="text-[11px] text-slate-400 leading-tight">{category.detail}</p>
                )}
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3.5">
                        <div className="w-12 h-9 rounded-lg border border-slate-100 bg-white flex items-center justify-center p-1 shrink-0 overflow-hidden">
                          {logo ? (
                            <Image
                              src={logo}
